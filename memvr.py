...
def set_visual_mask(model, mask):
    """设置视觉mask到模型"""
    model.model._visual_token_mask = mask
...
def forward(
    ...
    # ours
    visual_token_mask = getattr(self, '_visual_token_mask', None)
    # print("visual_token_mask", visual_token_mask)
    ...
    for decoder_layer in self.layers:
        ...
        if entropy-past_entropy > entropy_threshold and visual_retracing_event == False and layer > starting_layer and layer < ending_layer and apply_memvr: #激活重看极致
            ...
            if visual_token_mask is not None:
                # print("visual_token_mask is not None")
                visual_token = visual_token[visual_token_mask.bool()]
...
def activate_memvr(
    ...
    """
    Activate or deactivate MemVR in the model.
    """
    if enable:
        self.model.layers[0].mlp.apply_memvr = True
    else:
        self.model.layers[0].mlp.apply_memvr = False
