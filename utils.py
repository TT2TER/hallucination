...
def att_from_att_image_adaptive(att_map):
    ...
    Args:
        att_map: 一个 numpy array，表示 attention map（例如 24x24 或 16x16）

    Returns:
        mask: 一个  numpy array，形状与 att_map 相同。超过平均 attention 的位置为 1，否则为 0。
    """
...
def bbox_from_att_image_adaptive_hf(att_map, image_size, bbox_size=336):
    """
    Generates an adaptive bounding box for original image from an attention map.
