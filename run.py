...
def vicrop_qa_hf(model_name, method_name, image_path, question, model, processor, short_question):
    """
    ...
    if model_name == "llava":
        ...
        if method_name == 'grad_att':
            att_map = gradient_attention_llava_hf(image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(att_map, image.size, bbox_size)
        
        elif method_name == 'grad_att_high':
            att_maps = high_res(gradient_attention_llava_hf, image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(att_maps, image.size, bbox_size)

        #------------------------------------------------------------------------------------------------------------------------------------
        elif method_name == 'rel_att':
            att_map = rel_attention_llava_hf(image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(att_map, image.size, bbox_size)

        elif method_name == 'rel_att_high':
            att_maps = high_res(rel_attention_llava_hf, image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(att_maps, image.size, bbox_size)

        #------------------------------------------------------------------------------------------------------------------------------------
        elif method_name == 'pure_grad':
            grad = pure_gradient_llava_hf(image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(grad, image.size, bbox_size)

        elif method_name == 'pure_grad_high':
            grads = high_res(pure_gradient_llava_hf, image, short_prompt, general_prompt, model, processor)
            bbox = bbox_from_att_image_adaptive_hf(grads, image.size, bbox_size)
...
def vicrop_qa(model_name, method_name, image_path, question, model, tokenizer, image_processor, short_question, look_rate):
    ...
    if model_name == "llava":
        ...
        input_ids = tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).to(model.device)
        # print("token",IMAGE_TOKEN_INDEX)
        ...
        #------------------------------------------------------------------------------------------------------------------------------------
            ...
            att_map = rel_attention_llava(image, short_prompt, general_prompt, model, tokenizer, image_processor)
            bbox = bbox_from_att_image_adaptive(att_map, image.size, bbox_size)
            att_1d = att_map.reshape(-1)
            mask = att_from_att_image_adaptive(att_1d)

        elif method_name == 'rel_att_high':
            att_maps = high_res(rel_attention_llava, image, short_prompt, general_prompt, model, tokenizer, image_processor)
            bbox = bbox_from_att_image_adaptive(att_maps, image.size, bbox_size)
        
        #------------------------------------------------------------------------------------------------------------------------------------
        elif method_name == 'pure_grad':
            grad = pure_gradient_llava(image, short_prompt, general_prompt, model, tokenizer, image_processor)
